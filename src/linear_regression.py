...
def build_precision_two_complement_fixed_point(num_bits: int = 32, point_position: float = 0.5) -> np.array:
...
def build_precision_matrix(num_weights: np.array, precision: np.array) -> np.array:
    return np.kron(np.identity(num_weights), precision.T)


def build_augmented_feature_matrix(feature_matrix: np.array) -> np.array:
    bias = np.array([np.ones(feature_matrix.shape[0])]).T
    return np.concatenate([bias, feature_matrix], axis=1)


def build_Q(precision_matrix: np.array, feature_matrix: np.array, target_matrix: np.array) -> np.array:
    augmented_feature_matrix = build_augmented_feature_matrix(feature_matrix)
...
def get_weights(precision_matrix: np.array, binary_weights: np.array) -> np.array:
